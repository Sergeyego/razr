...
QVariant DbTableModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role) {
        case Qt::DisplayRole:
            value=modelData->column(index.column())->relation? modelData->column(index.column())->relation->data(modelData->value(index.row(),index.column()).toString())
                                                             : modelData->value(index.row(),index.column());
...
bool DbTableModel::insertDb()
{
    ...
    query.prepare(qu);
    for (int i=0; i<modelData->columnCount(); i++){
        if (!modelData->column(i)->isSerial && !QVariant(tmpRow[i]).isNull()){
            QVariant val;
...
bool DbTableModel::updateDb()
{
    ...
    for (int i=0; i<modelData->columnCount(); i++){
        if(newRow[i]!=oldRow[i]){
            if ((modelData->column(i)->type==TYPE_INT || modelData->column(i)->type==TYPE_DOUBLE) && newRow.at(i).toString()==""){
